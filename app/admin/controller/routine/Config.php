            ...
            foreach ($all as $item) {
                if (array_key_exists($item->name, $data)) {
                    ...
                    if ($item->name == 'backend_entrance') {
                        ...
                        if (!$adminBaseContent) {
                            $this->error(__('Configuration write failed: %s', [$this->filePath['webAdminBase']]));
                        }
                        ...
                        if (!$appConfigContent) {
                            $this->error(__('Configuration write failed: %s', [$this->filePath['appConfig']]));
                        }
                        ...
                        if ($newBackendEntrance != 'admin') {
                            ...
                            if (!$backendEntranceStub) {
                                $this->error(__('Configuration write failed: %s', [$this->filePath['backendEntranceStub']]));
                            }

                            $result = @file_put_contents($newBackendEntranceFile, $backendEntranceStub);
                            if (!$result) {
                                $this->error(__('Configuration write failed: %s', [$newBackendEntranceFile]));
                            }
                        }
                    }
                }
